...
fun <T> Grid<T>.toGraph(predicate: (Position) -> Boolean) = Graph<Position>().also { graph ->
    this.positions.forEach { pos ->
        if (predicate(pos)) {
            graph.add(pos)
            for (neighbor in pos.walk(Direction.Cardinals).map { it.pos }.filter { it in this }) {
...
fun Grid<Char>.toGraph(pattern: String = ".") = this.toGraph { this[it] in pattern }
